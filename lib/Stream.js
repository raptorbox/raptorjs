...
class Stream extends Base {
    ...
    list(stream, page, size, sort) {
        ...
        return this.getClient().get(this.route("STREAM_PULL", stream.deviceId, stream.name) + util.createQueryString(page, size, sort))
            .then((list) => Promise.resolve(require("./pager").create(list, (record) => new Record(record, stream))))
    }
    ...
    search(stream, q) {
        ...
        return this.getClient().post(this.route("STREAM_SEARCH", stream.deviceId, stream.name), q)
            .then((list) => Promise.resolve(require("./pager").create(list, (record) => new Record(record, stream))))
    }
